...
#show: project.with(
  ...
  mentor: "慕冬亮",
  abstract_zh: [
    软件容易出现安全漏洞。由于依赖于人工标记的规范，用于检测它们的程序分析工具在实践中效果有限。大型语言模型（或 LLMs）已经显示出令人印象深刻的代码生成能力，但它们无法对代码进行复杂的推理来检测此类漏洞，尤其是因为此任务需要整个存储库分析。我们提出了 IRIS，这是一种神经符号方法，它系统地将静态分析与LLM相结合，以执行整个存储库推理以进行安全漏洞检测。具体来说，IRIS 利用LLMs推断污染规格并执行上下文分析，从而减轻对人工规格和检查的需求。为了进行评估，我们策划了一个新的数据集 CWE-Bench-Java，其中包含实际 Java 项目中的 120 个手动验证的安全漏洞。最先进的静态分析工具 CodeQL 仅检测到其中的 27 个漏洞，而带有 GPT-4 的 IRIS 检测到 55 个（+28），并将 CodeQL 的平均错误发现率提高了 5 个百分点。此外，IRIS 还识别了 6 个现有工具无法发现的以前未知的漏洞。
  ],
  keywords_zh: ("Neuro-Symbolic", "Program Analysis", "Security Vulnerability", "LLM"),
)

= 引言
安全漏洞对软件应用程序及其用户的安全构成重大威胁。仅在 2023 年，就报告了超过 29,000 个 CVE，比 2022 年高出近 4000 个 #bib_cite(<cvedetails>)。尽管发现漏洞的技术取得了进步，但检测漏洞极具挑战性。一种很有前途的技术是静态污点分析，广泛用于 GitHub CodeQL#bib_cite(<codeql>)、Facebook Infer #bib_cite(<fbinfer>)、Checker Framework#bib_cite(<checker>) 和 Snyk Code#bib_cite(<synkio>) 等流行工具中。然而，这些工具面临着一些挑战，极大地限制了它们在实践中的有效性和可及性。
...
*由于缺少第三方库 API 的污点规范而导致的漏报。*首先，静态污点分析主要依赖于第三方库 API 的规范，如 source、sink 或santizers。在实践中，开发人员和分析工程师必须根据他们的领域知识和 API 文档手动制定此类规范。这是一个费力且容易出错的过程，通常会导致缺少规范和对漏洞的分析不完整。此外，即使许多库可能存在此类规范，也需要定期更新这些规范，以捕获此类库的较新版本中的更改，并涵盖开发的新库。
...
*先前数据驱动方法在改进静态污点分析方面的局限性。*已经提出了许多技术来解决静态污染分析的挑战。例如，MERLIN#bib_cite(<livshits2009merlin>)，用于自动挖掘污点规格。最近的工作 Seldon#bib_cite(<chibotaru2019scalable>) 通过将污点规格推理问题表述为线性优化任务，提高了这种方法的可扩展性。但是，此类方法依赖于分析第三方库的代码来提取规范，这成本高昂且难以扩展。研究人员还开发了统计和基于学习的技术来减少误报警报 #bib_cite(<jung2005taming>, <heckman2009model>, <ranking2014finding>)。然而，这种方法在实践中仍然效果有限#bib_cite(<kang2022detecting>)。

大型语言模型（或 LLM） 在代码生成和摘要方面取得了令人印象深刻的进步。LLMs也被应用于与代码相关的任务，如程序修复 #bib_cite(<xia2023automated>)、代码转换 #bib_cite(<pan2024lost>)、测试生成#bib_cite(<lemieux2023codamosa>) 和静态分析 #bib_cite(<li2024enhancing>)。最近的研究 #bib_cite(<steenhoek2024comprehensive>, <khare2023understanding>) 评估LLMs了在方法级别检测漏洞的有效性，并表明LLM无法对代码进行复杂的推理，特别是因为它取决于方法在项目中使用的上下文。另一方面，最近的基准测试，如 SWE-Bench #bib_cite(<jimenez2023swe>) 表明，LLM在进行项目级推理方面也很差。因此，一个有趣的问题是是否可以将LLM与静态分析相结合来提高他们的推理能力。在这项工作中，我们在漏洞检测的背景下回答了这个问题，并做出了以下贡献：

*方法*：我们提出了 IRIS，这是一种用于漏洞检测的神经符号方法，它结合了静态分析和 LLM. @fig:img1 显示了 IRIS 的概述。给定一个要分析给定漏洞类别（或 CWE）的项目，IRIS 使用LLM挖掘特定于 CWE 的污点规范。IRIS 使用 CodeQL（一种静态污点分析工具）增强了此类规范。我们的直觉是因为LLMs已经看到了这种库 API 的大量用法，他们对不同 CWE 的相关 API 有了解。此外，为了解决静态分析的不精确性问题，我们提出了一种上下文分析技术，该技术LLMs可以减少误报并最大限度地减少开发人员的分类工作。我们的主要见解是，对 Prompt 中的代码上下文和路径敏感信息进行编码会从 LLMs中引出更可靠的推理。最后，我们的神经符号方法可以LLMs进行更精确的整个存储库推理，并最大限度地减少使用静态分析工具所涉及的人力。
...
== 漏洞检测
一旦我们从 获得所有 source 和 sink 规格LLM，下一步就是将其与静态分析引擎相结合，以检测给定项目中的易受攻击路径，即$"Unsanitized_Paths"(V_s, V_t)$。在这项工作中，我们使用 CodeQL#bib_cite(<codeql-web>) 来完成此步骤。CodeQL 将程序表示为数据流图，并提供一种类似于 Datalog#bib_cite(<smaragdakis2010using>) 的查询语言来分析此类图。许多安全漏洞可以使用用 CodeQL 编写的查询进行建模，并且可以针对从此类程序中提取的数据流图执行。给定项目$P$的数据流图$GG^P$，CWE 特定的源和接收器规范，以及对给定漏洞类$C$的查询，CodeQL 在程序中返回一组未经清理的路径。形式化地表示$ "CodeQL"(GG^P, S_("source")^C, S_("sink")^C, "Query"^C) = {"Path"_1, ..., "Path"_k} $
...
== 通过上下文分析对警报进行分类
推断污点规格只能解决部分挑战。我们观察到，虽然有助于发现许多新的 API 规范，但LLMs有时它们会检测到与所考虑的漏洞类别无关的规范，从而导致预测的源或接收器过多，从而导致许多虚假警报。就上下文而言，即使是几百个污点规范有时也可能产生数千个$"Unsanitized_Paths"(V_s, V_t)$，开发人员需要对其进行分类。为了减轻开发人员的负担，我们还开发了一种LLM基于 的过滤方法，$"FilterPath"（"Path"， GG， "LLM"， C） = "True"|"False"$，通过利用基于上下文和自然语言的信息，将$GG$中检测到的易受攻击路径 （Path）分类为真阳性或假阳性。
...
我们希望 LLM 以JSON 格式响应，并提供最终裁决以及对裁决的解释。JSON 格式会提示 在LLM做出最终裁决之前生成解释，因为已知在推理过程之后提出判决会产生更好的结果。此外，如果判定为 false，则我们要求 指示 LLM source 或 sink 是误报，这有助于修剪其他路径，从而节省对LLM的调用次数。

== 评估指标
我们使用三个关键指标评估 IRIS 及其基线的性能：检测到的漏洞数量 （\#Detected）、平均错误发现率 （AvgFDR） 和平均 F1 （AvgF1）。为了进行评估，我们假设我们有一个数据集$D = {P_1， . . . ， P_n}$，其中每个$P_i$都是一个 Java 项目，并且已知包含至少一个漏洞。项目$P$的标签以一组关键程序点 $V_("Vul")^P= {V_1， . . . . ， V_n}$的形式提供，易受攻击的路径应该通过这些点，由$"Path" inter V_("Vul")^P eq.not nothing$表示。在实践中，这些通常是可以从每个漏洞报告中收集的修补方法。如果检测到的至少一个易受攻击的路径通过给定漏洞的固定位置，则我们认为检测到的漏洞。设$"Paths"^P$前一阶段每个项目 P 检测到的路径集。指标的形式化定义如下：
...
为了评估我们的方法，我们需要一个 Java 项目的易受攻击版本的数据集，该数据集具有几个重要特征：1） 每个基准测试都应该有相关的*漏洞元数据*，例如 CWE ID、CVE ID、修复提交和易受攻击的项目版本，2） 数据集中的每个项目都必须是*可编译的*，这是静态分析和数据流图提取的关键要求， 3） 项目必须是*真实的*，与综合基准相比，它们通常更复杂，因此难以分析， 4） 最后，必须*验证*项目中每个漏洞及其位置（例如方法），以便这些信息可用于对漏洞检测工具的稳健评估。遗憾的是，没有现有的数据集满足所有这些要求。
